...
def _get(path: str, client: httpx.Client | None = None) -> dict:
    if client is None:
        with httpx.Client(timeout=15) as owned_client:
            return _get(path, client=owned_client)
...
def _active_timetables(client: httpx.Client) -> list[dict]:
    return _get("/api/v1/timetable/?status=active", client=client).get("data", [])


def _timetable_entries(timetable_id: str, client: httpx.Client) -> list[dict]:
    detail = _get(f"/api/v1/timetable/{timetable_id}", client=client)
    return detail.get("data", {}).get("entries", [])
...
def overview():
    ...
    try:
        with httpx.Client(timeout=15) as client:
            timetables = _get("/api/v1/timetable/", client=client)
            rooms = _get("/api/v1/rooms", client=client)
            lecturers = _get("/api/v1/lecturers", client=client)
            courses = _get("/api/v1/courses", client=client)
...
def room_utilization():
    ...
    try:
        with httpx.Client(timeout=15) as client:
            rooms = _get("/api/v1/rooms", client=client).get("data", [])
            timetables = _active_timetables(client=client)
            room_counts = {r["id"]: {"name": r["name"], "capacity": r["capacity"], "bookings": 0} for r in rooms}
            for tt in timetables:
                for entry in _timetable_entries(tt["id"], client=client):
...
def lecturer_workload():
    ...
    try:
        with httpx.Client(timeout=15) as client:
            lecturers = _get("/api/v1/lecturers", client=client).get("data", [])
